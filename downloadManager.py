...
def cleanUpAfterCrash():
    ...
    # If a file is still in the queue as 'downloading'
    # But there is a file of the same name in the temp directory,
    # set the jobs status to Queued and remove the incomplete file
...
def queueManager():
    ...
    download_semaphore = threading.BoundedSemaphore(value=MAX_NUMBER_OF_DOWNLOADS)  # noqa
        ...
        if len(getDownloads()) < MAX_NUMBER_OF_DOWNLOADS:
            ...
            for files in files_in_active_status:
                if files[2] == "Queued":
                    ...
                    url = files[1]
                    defaultFileName = makeDownloadFileName(url)
                    downloadContainer = DownloadThread(url,
                                                       TEMP_LOCATION,
                                                       defaultFileName,
                                                       uniqueID,
                                                       download_semaphore)
                    # Set thread daemon flag,
                    # if server shuts down thread will terminate
                ...
                # Don't create new threads if MAX_NUMBER_OF_DOWNLOADS
                # has been reached.
                if len(getDownloads()) >= MAX_NUMBER_OF_DOWNLOADS:
                    break
...
def purgeDownloads():
    ...
    for download_thread in ACTIVE_DOWNLOADS:
        if not download_thread.isAlive():
            # Queue the thread for deletion,
            # if it's deleted immediately iteration
